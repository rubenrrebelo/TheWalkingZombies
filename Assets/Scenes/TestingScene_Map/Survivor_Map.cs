...
public class Survivor_Map: MonoBehaviour {

    private bool _isExecutingAPlansIntruction = false;
	...
	private void randomMove(){
        ...
        //Debug.Log("MOVE " + CurrentDestination);
		/**/
	}
	...
	private string SurvivorAction(GameObject survivor){
        return survivor.GetComponent<Survivor_Map>().getState();
	...
	private Vector3 anySurvivorCollecting(){
		...
		foreach(GameObject survivor in _survivorsInSight){ //check each survivor around to see if they are collecting
            if (survivor.GetComponent<Survivor_Map>().getState().Equals(COLLECTING))
            {
	...
	private Vector3 anySurvivorAttacking(){
		...
		foreach(GameObject survivor in _survivorsInSight){ //check each survivor around to see if they are attacking
            if (survivor.GetComponent<Survivor_Map>().getState().Equals(ATTACKING))
            {
				if(survivorInNeed == null){
					survivorInNeed = survivor;
	...
	private GameObject healthiestSurvivor(){
		...
		foreach (GameObject newSurvivor in _survivorsInSight) {
			...
			float newSurvivorHealth = newSurvivor.GetComponent<Survivor_Map>()._healthLevel;
	...
	private void demandNewTank(){
		_partyLeader.GetComponent<Survivor_Map> ().reRollTank (this.gameObject);
		_isTank = false;
	}
            ...
            if (!_dead)
            {
                ...
                if (_isPartyLeader)
                {
                    if (_tank == null)
                    {
                        ...
                        _tank.GetComponent<Survivor_Map>()._isTank = true;
    ...
    IEnumerator CicloBDI()
    {
        ...
        while (true)
        {
            ...
            Debug.Log("Plan " + i + " #" + thePlan.Count);

            while (j < thePlan.Count)
            {
                Debug.Log("Plano " + i + " instrucao " + j);

                instruction = thePlan[j];
